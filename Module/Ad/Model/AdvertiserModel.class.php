...
class AdvertiserModel extends Model {
    ...
    protected $_validate = array(
        array('name', 'require', '广告商用户名！', self::EXISTS_VALIDATE), //默认情况下用正则进行验证
        array('name', '', '广告商用户名已经存在,请尝试更换广告商用户名！', 0, 'unique', 1),
        array('email', 'require', '广告商邮箱不能为空！', self::EXISTS_VALIDATE), //默认情况下用正则进行验证
        array('portrait', 'require', '广告商头像不能为空！', self::EXISTS_VALIDATE), //默认情况下用正则进行验证
...
//        array('contacts_wechat', '', '联系人微信已经存在,请尝试更换联系人微信！', 0, 'unique', 1),
    ...
    protected $_auto = array(
        array('product_num', '0', self::MODEL_INSERT),
        array('advertiser_status', '0', self::MODEL_INSERT),
