...
(function () {
    ...
    function initTabs() {
        ...
        /*添加在线管理开关*/
        if (!editor.getOpt('onlineImage')) {
            setTabFocus('upload');
            $G('tabhead').children[2].style.display = 'none';
        }
        if (editor.getOpt('insertAlign') === false) {
            $('.alignBar').hide();
        }
    }
        ...
        initUploader: function () {
            ...
            uploader.on('uploadBeforeSend', function (file, data, header) {
                ...
                $.ajax({
                    ...
                    async: false,
                    data: {key: data['key']},
                    url: window.top._plugin_editor_token_,
