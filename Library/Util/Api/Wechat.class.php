...
class Wechat {
    ...
    public function getRevCardGet() {
        ...
        if (isset($this->_receive['FromUserName']))      //领取人的openid
        ...
        if (isset($this->_receive['FriendUserName']))    //赠送方账号（一个OpenID），"IsGiveByFriend”为1时填写该参数。
            $array['FriendUserName'] = $this->_receive['FriendUserName'];
        if (isset($this->_receive['CreateTime']))        //消息创建时间 （整型）。
            $array['CreateTime'] = $this->_receive['CreateTime'];
        ...
        if (isset($this->_receive['OldUserCardCode']) && !empty($this->_receive['OldUserCardCode'])) //转赠前的code序列号。
            $array['OldUserCardCode'] = $this->_receive['OldUserCardCode'];
        if (isset($this->_receive['OuterId']) && !empty($this->_receive['OuterId'])) //领取场景值，用于领取渠道数据统计。可在生成二维码接口及添加JS API接口中自定义该字段的整型值。
            $array['OuterId'] = $this->_receive['OuterId'];
        if (isset($array) && count($array) > 0) {
            return $array;
        } else {
            return false;
    ...
    /**
     * 获取卡券事件推送 - 卡券核销
     * 当Event为 user_consume_card(用户核销卡券)
     * @return array|boolean
     */
    public function getRevCardConsume() {
        if (isset($this->_receive['CardId']))     //卡券 ID
            $array['CardId'] = $this->_receive['CardId'];
        if (isset($this->_receive['FromUserName']))      //发送方帐号（一个OpenID）
            $array['FromUserName'] = $this->_receive['FromUserName'];
        if (isset($this->_receive['CreateTime']))      //消息创建时间 （整型）。
            $array['CreateTime'] = $this->_receive['CreateTime'];
        if (isset($this->_receive['UserCardCode']) && !empty($this->_receive['UserCardCode'])) //code 序列号。自定义 code 及非自定义 code的卡券被领取后都支持事件推送。
            $array['UserCardCode'] = $this->_receive['UserCardCode'];
        if (isset($this->_receive['ConsumeSource']))      //核销来源。支持开发者统计API核销（FROM_API）、公众平台核销（FROM_MP）、卡券商户助手核销（FROM_MOBILE_HELPER）（核销员微信号）
            $array['ConsumeSource'] = $this->_receive['ConsumeSource'];
        if (isset($array) && count($array) > 0) {
            return $array;
        } else {
            return false;
     ...
     * @return array|boolean
     */
    public function getRevStorePass() {
        if (isset($this->_receive['UniqId']))     //商户自己内部ID，即字段中的sid
            $array['UniqId'] = $this->_receive['UniqId'];
        if (isset($this->_receive['PoiId']))      //微信的门店ID，微信内门店唯一标示ID
            $array['PoiId'] = $this->_receive['PoiId'];
        if (isset($this->_receive['Result']))      //审核结果，成功succ 或失败fail
            $array['Result'] = $this->_receive['Result'];
        if (isset($this->_receive['Msg']))      //成功的通知信息，或审核失败的驳回理由
            $array['Msg'] = $this->_receive['Msg'];
        if (isset($array) && count($array) > 0) {
            return $array;
        } else {
            return false;
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    protected function log($log) {
        if ($this->debug) {
            if (function_exists($this->logcallback)) {
                is_array($log) && $log = print_r($log, true);
    ...
    public function checkAuth($appid = '', $appsecret = '', $token = '') {
        ...
        $rs = S($cachename);
        if (!empty($rs)) {
            return $this->access_token = $rs;
        }
        $result = $this->http_get(self::API_URL_PREFIX . '/token?grant_type=client_credential&appid=' . $appid . '&secret=' . $appsecret);
        if ($result) {
            ...
            S($cachename, $this->access_token, 6000);
            return $this->access_token;
        }
        return false;
    }
    ...
    public function resetJsTicket($appid = '') {
        ...
        $this->jsapi_ticket = '';
        $cachename = 'wechat_jsapi_ticket_' . $appid;
        S($cachename, null);
        return true;
    }
    ...
    public function getJsTicket($appid = '', $jsapi_ticket = '') {
        ...
        if ($result) {
            ...
            S($cachename, $this->jsapi_ticket, 6000);
            return $this->jsapi_ticket;
        }
        return false;
    }
    ...
    public function createMenu($data) {
        ...
        $result = $this->http_post(self::API_URL_PREFIX . '/menu/create?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getMenu() {
        ...
        $result = $this->http_get(self::API_URL_PREFIX . '/menu/get?access_token=' . $this->access_token);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || isset($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function uploadMedia($data, $type) {
        ...
        $result = $this->http_post(self::API_URL_PREFIX . '/media/upload?access_token=' . $this->access_token . '&type=' . $type, $data, true);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getMedia($media_id, $is_video = false) {
        ...
        $result = $this->http_get($url_prefix . '/media/get?access_token=' . $this->access_token . '&media_id=' . $media_id);
        if ($result) {
            $json = json_decode($result, true);
    ...
    public function uploadForeverMedia($data, $type, $is_video = false, $video_info = array()) {
        ...
        $result = $this->http_post(self::API_URL_PREFIX . '/material/add_material?access_token=' . $this->access_token . '&type=' . $type, $data, true);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function uploadForeverArticles($data) {
        ...
        $result = $this->http_post(self::API_URL_PREFIX . '/material/add_news?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function updateForeverArticles($media_id, $data, $index = 0) {
        ...
        $result = $this->http_post(self::API_URL_PREFIX . '/material/update_news?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getForeverMedia($media_id, $is_video = false) {
        ...
        //如果要获取的素材是视频文件时，不能使用https协议，必须更换成http协议
        //$url_prefix = $is_video?str_replace('https','http',self::API_URL_PREFIX):self::API_URL_PREFIX;
        $result = $this->http_post(self::API_URL_PREFIX . '/material/get_material?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (isset($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function delForeverMedia($media_id) {
        ...
        $data = array('media_id' => $media_id);
        $result = $this->http_post(self::API_URL_PREFIX . '/material/del_material?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getForeverList($type, $offset, $count) {
        ...
        $result = $this->http_post(self::API_URL_PREFIX . '/material/batchget_material?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (isset($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getForeverCount() {
        ...
        $result = $this->http_get(self::API_URL_PREFIX . '/material/get_materialcount?access_token=' . $this->access_token);
        if ($result) {
            $json = json_decode($result, true);
            if (isset($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function uploadArticles($data) {
        ...
        $result = $this->http_post(self::API_URL_PREFIX . '/media/uploadnews?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function uploadMpVideo($data) {
        ...
        $result = $this->http_post(self::UPLOAD_MEDIA_URL . '/media/uploadvideo?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function deleteMassMessage($msg_id) {
        ...
        $result = $this->http_post(self::API_URL_PREFIX . '/message/mass/delete?access_token=' . $this->access_token, self::json_encode(array('msg_id' => $msg_id)));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function previewMassMessage($data) {
        ...
        $result = $this->http_post(self::API_URL_PREFIX . '/message/mass/preview?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function queryMassMessage($msg_id) {
        ...
        $result = $this->http_post(self::API_URL_PREFIX . '/message/mass/get?access_token=' . $this->access_token, self::json_encode(array('msg_id' => $msg_id)));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getQRCode($scene_id, $type = 0, $expire = 1800) {
        ...
        $data = array(
            'action_name' => $type ? ($type == 2 ? "QR_LIMIT_STR_SCENE" : 1) : 0,
        ...
        $result = $this->http_post(self::API_URL_PREFIX . '/qrcode/create?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getShortUrl($long_url) {
        ...
        $result = $this->http_post(self::API_URL_PREFIX . '/shorturl?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getUserList($next_openid = '') {
        ...
        $result = $this->http_get(self::API_URL_PREFIX . '/user/get?access_token=' . $this->access_token . '&next_openid=' . $next_openid);
        if ($result) {
            $json = json_decode($result, true);
            if (isset($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getUserInfo($openid) {
        ...
        $result = $this->http_get(self::API_URL_PREFIX . '/user/info?access_token=' . $this->access_token . '&openid=' . $openid);
        if ($result) {
            $json = json_decode($result, true);
            if (isset($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function updateUserRemark($openid, $remark) {
        ...
        $result = $this->http_post(self::API_URL_PREFIX . '/user/info/updateremark?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getGroup() {
        ...
        $result = $this->http_get(self::API_URL_PREFIX . '/groups/get?access_token=' . $this->access_token);
        if ($result) {
            $json = json_decode($result, true);
            if (isset($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getUserGroup($openid) {
        ...
        $result = $this->http_post(self::API_URL_PREFIX . '/groups/getid?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function createGroup($name) {
        ...
        $result = $this->http_post(self::API_URL_PREFIX . '/groups/create?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function updateGroup($groupid, $name) {
        ...
        $result = $this->http_post(self::API_URL_PREFIX . '/groups/update?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function updateGroupMembers($groupid, $openid) {
        ...
        $result = $this->http_post(self::API_URL_PREFIX . '/groups/members/update?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function batchUpdateGroupMembers($groupid, $openid_list) {
        ...
        $result = $this->http_post(self::API_URL_PREFIX . '/groups/members/batchupdate?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function sendCustomMessage($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_URL_PREFIX . '/message/custom/send?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getOauthAccessToken() {
        ...
        if (!$code)
            return false;
        $result = $this->http_get(self::API_BASE_URL_PREFIX . '/sns/oauth2/access_token?appid=' . $this->appid . '&secret=' . $this->appsecret . '&code=' . $code . '&grant_type=authorization_code');
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getOauthRefreshToken($refresh_token) {
        $result = $this->http_get(self::API_BASE_URL_PREFIX . '/sns/oauth2/refresh_token?appid=' . $this->appid . '&grant_type=refresh_token&refresh_token=' . $refresh_token);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getOauthUserinfo($access_token, $openid) {
        $result = $this->http_get(self::API_BASE_URL_PREFIX . '/sns/userinfo?access_token=' . $access_token . '&openid=' . $openid);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getOauthAuth($access_token, $openid) {
        $result = $this->http_get(self::API_BASE_URL_PREFIX . '/sns/auth?access_token=' . $access_token . '&openid=' . $openid);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function setTMIndustry($id1, $id2 = '') {
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_URL_PREFIX . '/message/template/api_set_industry?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function addTemplateMessage($tpl_id) {
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_URL_PREFIX . '/message/template/api_add_template?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function sendTemplateMessage($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_URL_PREFIX . '/message/template/send?access_token=' . $this->access_token, urldecode(self::json_encode($data)));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getCustomServiceMessage($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_URL_PREFIX . '/customservice/getrecord?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function createKFSession($openid, $kf_account, $text = '') {
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/customservice/kfsession/create?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function closeKFSession($openid, $kf_account, $text = '') {
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/customservice/kfsession/close?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function addKFAccount($account, $nickname, $password) {
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/customservice/kfaccount/add?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function updateKFAccount($account, $nickname, $password) {
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/customservice/kfaccount/update?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function querySemantic($uid, $query, $category, $latitude = 0, $longitude = 0, $city = "", $region = "") {
        ...
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/semantic/semproxy/search?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function createCard($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/card/create?access_token=' . $this->access_token, self::json_encode(array('card' => $data)));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function updateCard($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/card/update?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function delCard($card_id) {
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/card/delete?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getCardInfo($card_id) {
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/card/get?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getCardColors() {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_get(self::API_BASE_URL_PREFIX . '/card/getcolors?access_token=' . $this->access_token);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getCardLocations($offset = 0, $count = 0) {
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/card/location/batchget?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function addCardLocations($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/card/location/batchadd?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function createCardQrcode($card_id, $code = '', $openid = '', $expire_seconds = 0, $is_unique_code = false, $balance = '') {
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/card/qrcode/create?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function consumeCardCode($code, $card_id = '') {
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/card/code/consume?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function decryptCardCode($encrypt_code) {
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/card/code/decrypt?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function checkCardCode($code) {
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/card/code/get?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getCardIdList($offset = 0, $count = 50) {
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/card/batchget?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function updateCardCode($code, $card_id, $new_code) {
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/card/code/update?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function unavailableCardCode($code, $card_id = '') {
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/card/code/unavailable?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function modifyCardStock($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/card/modifystock?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function activateMemberCard($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/card/membercard/activate?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function updateMemberCard($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/card/membercard/updateuser?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function updateLuckyMoney($code, $balance, $card_id = '') {
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/card/luckymoney/updateuserbalance?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function setCardTestWhiteList($openid = array(), $user = array()) {
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/card/testwhitelist/set?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function applyShakeAroundDevice($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/shakearound/device/applyid?access_token=' . $this->access_token, self::json_encode($data));
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
    ...
    public function searchShakeAroundDevice($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/shakearound/device/search?access_token=' . $this->access_token, self::json_encode($data));
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
    ...
    public function bindLocationShakeAroundDevice($device_id, $poi_id, $comment, $uuid, $major, $minor) {
        ...
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/shakearound/device/bindlocation?access_token=' . $this->access_token, self::json_encode($data));
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
    ...
    public function bindPageShakeAroundDevice($device_id, $page_ids = array(), $bind = 1, $append = 1, $uuid, $major, $minor) {
        ...
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/shakearound/device/bindpage?access_token=' . $this->access_token, self::json_encode($data));
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
    ...
    public function addShakeAroundPage($title, $description, $icon_url, $page_url, $comment = '') {
        ...
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/shakearound/page/add?access_token=' . $this->access_token, self::json_encode($data));
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * @return boolean
     */
    public function addShakeArounMedia($imgname) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $data = array(
            "media" => '@' . $imgname,
        );
        $result = $this->http_post('https://api.weixin.qq.com/shakearound/material/add?access_token=' . $this->access_token, $data, true);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function updateShakeAroundPage($page_id, $title, $description, $icon_url, $page_url, $comment = '') {
        ...
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/shakearound/page/update?access_token=' . $this->access_token, self::json_encode($data));
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
    ...
    public function searchShakeAroundPage($page_ids = array(), $begin = 0, $count = 1) {
        ...
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/shakearound/page/search?access_token=' . $this->access_token, self::json_encode($data));
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
    ...
    public function deleteShakeAroundPage($page_ids = array()) {
        ...
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/shakearound/page/delete?access_token=' . $this->access_token, self::json_encode($data));
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
    ...
    /**
     * 获取设备信息，包括UUID、major、minor，以及距离、openID 等信息。
    ...
    public function getShakeInfoShakeAroundUser($ticket) {
        ...
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/shakearound/user/getshakeinfo?access_token=' . $this->access_token, self::json_encode($data));
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
    ...
    public function deviceShakeAroundStatistics($device_id, $begin_date, $end_date, $uuid, $major, $minor) {
        ...
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/shakearound/statistics/device?access_token=' . $this->access_token, self::json_encode($data));
        $this->log($result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
    ...
    /**
     * 获取已在微信公众平台添加的门店列表
     * @param type $begin 开始拉取的偏移，默认为0从头开始
     * @param type $limit 拉取的数量，默认为20条
     * @return boolean
     */
    public function getPoiList($begin = 0, $limit = 20) {
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_URL_PREFIX . '/poi/getpoilist?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getWxCategory() {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_get(self::API_URL_PREFIX . '/api_getwxcategory?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function addPoi($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_URL_PREFIX . '/poi/addpoi?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getPoi($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_URL_PREFIX . '/poi/getPoi?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function updatePoi($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_URL_PREFIX . '/poi/updatepoi?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function delPoi($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_URL_PREFIX . 'poi/delpoi?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function upLoadImg($data) {
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_URL_PREFIX . '/media/uploadimg?access_token=' . $this->access_token, $data, true);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @return type
     */
    public function getCardApiTicket($appid = '') {
        $ticket = S('card_api_ticket_' . $appid);
        ...
        if ($ticket) {
            ...
            $url = self::API_URL_PREFIX . "/ticket/getticket?access_token=" . $this->access_token . "&type=wx_card";
            $res = json_decode($this->http_get($url));
            $ticket = $res->ticket;
            if ($ticket) {
                S('card_api_ticket_' . $appid, $ticket, 7200);
            }
            if (empty($ticket)) {
